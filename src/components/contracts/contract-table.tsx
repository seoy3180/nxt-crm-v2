      ...
      <Table>
        <TableHeader>
          <TableRow className="bg-zinc-50">
            ...
            <TableHead className="h-10 w-[140px] px-2 text-xs font-semibold text-zinc-500">
              고객
            </TableHead>
            <TableHead className="h-10 w-[80px] px-2 text-xs font-semibold text-zinc-500">
              타입
            </TableHead>
            <TableHead className="h-10 w-[100px] px-2 text-xs font-semibold text-zinc-500">
              단계
            </TableHead>
            <TableHead className="h-10 w-[110px] px-2 text-xs font-semibold text-zinc-500">
              사내 담당자
            </TableHead>
            <TableHead className="h-10 w-[110px] px-2 text-xs font-semibold text-zinc-500">
              고객사 담당자
        ...
        <TableBody>
          {contracts.length === 0 ? (
            ...
            contracts.map((contract) => (
              ...
              >
                <TableCell className="px-6 text-sm font-medium text-zinc-900 break-words whitespace-normal">
                  {contract.name}
                </TableCell>
                <TableCell className="px-2 text-[13px] text-zinc-500 break-words whitespace-normal">
                  {contract.client_name ?? '-'}
                </TableCell>
                <TableCell className="px-2">{getTypeBadge(contract.type)}</TableCell>
                <TableCell className="px-2">
                  {getStageBadge(contract.stage, contractType)}
                </TableCell>
                <TableCell className="px-2 text-[13px] text-zinc-500">
                  {contract.assigned_to_name ?? '-'}
                </TableCell>
                <TableCell className="px-2 text-[13px] text-zinc-500">
                  {contract.client_contact_name ?? '-'}
                </TableCell>
                <TableCell className="px-2 text-[13px] font-medium text-zinc-900">
