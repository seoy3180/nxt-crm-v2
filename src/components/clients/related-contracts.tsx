      ...
      <div className="flex items-center justify-between">
        <h3 className="pl-2 text-lg font-semibold text-zinc-900">
          ...
          <span className="text-zinc-400">
            ({contracts.length}건)
            {childClientIds && childClientIds.length > 0 ? ' — 하위 고객 포함' : ''}
          </span>
        </h3>
      </div>
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-zinc-50">
              {hasChildren && (
                <TableHead className="h-10 w-[140px] px-4 text-xs font-semibold text-zinc-500">
                  고객
                </TableHead>
              )}
              <TableHead className="h-10 px-4 text-xs font-semibold text-zinc-500">
                계약명
              </TableHead>
              <TableHead className="h-10 w-[80px] px-4 text-center text-xs font-semibold text-zinc-500">
                타입
              </TableHead>
              <TableHead className="h-10 w-[100px] px-4 text-center text-xs font-semibold text-zinc-500">
                단계
              </TableHead>
              <TableHead className="h-10 w-[120px] px-4 text-center text-xs font-semibold text-zinc-500">
                사내 담당자
              </TableHead>
              <TableHead className="h-10 w-[120px] px-4 text-center text-xs font-semibold text-zinc-500">
                고객사 담당자
            ...
            {contracts.length === 0 ? (
              ...
              contracts.map((c) => (
                <TableRow
                  key={c.id}
                  tabIndex={0}
                  className="h-11 cursor-pointer border-b border-zinc-100 transition-colors hover:bg-zinc-50 focus:bg-zinc-50 focus:outline-none"
                  onClick={() => router.push(`${basePath}/contracts/${c.id}`)}
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') router.push(`${basePath}/contracts/${c.id}`);
                  }}
                >
                  {hasChildren && (
                    <TableCell className="w-[140px] px-4 text-sm font-medium text-zinc-900 break-words whitespace-normal">
                      {c.client_name ?? '-'}
                    </TableCell>
                  )}
                  ...
                  <TableCell className="w-[80px] px-4 text-center">
                    {(() => {
                      ...
                      return (
                        <span
                          className={`inline-block rounded px-2 py-0.5 text-[11px] font-semibold ${colors[c.type] ?? 'bg-zinc-100 text-zinc-600'}`}
                        >
                  ...
                  <TableCell className="w-[100px] px-4 text-center">
                    <span
                      className={`inline-block rounded px-2 py-0.5 text-[11px] font-semibold ${getStageColor(c.stage)}`}
                    >
                      {getStageLabel(c.stage, c.type)}
                    </span>
                  </TableCell>
                  <TableCell className="w-[120px] px-4 text-center text-[13px] text-zinc-500">
                    {c.assigned_to_name ?? '-'}
                  </TableCell>
                  <TableCell className="w-[120px] px-4 text-center text-[13px] text-zinc-500">
                    {c.client_contact_name ?? '-'}
                  </TableCell>
                  <TableCell className="w-[120px] px-4 text-center text-[13px] font-medium text-zinc-900">
