...
export function getStageBadge(stage: string | null, contractType: string) {
  ...
  return (
    <span
      className={`inline-block rounded px-2 py-0.5 text-[11px] font-semibold ${getStageColor(stage)}`}
    >
      {label}
    </span>
  );
...
export function renderCellValue(
  ...
  if (col.key === 'clientName' && row.clientId) {
    return (
      <Link
        href={`${opts.basePath}/clients/${row.clientId}`}
  ...
  if (col.key === 'actions') {
    return (
      <Link
        href={`${opts.basePath}/contracts/${row.id}`}
  ...
  if (col.key === 'billingOn') {
    ...
    return (
      <span
        className={`inline-block rounded px-2 py-0.5 text-[11px] font-semibold ${on ? 'bg-emerald-100 text-emerald-600' : 'bg-zinc-100 text-zinc-500'}`}
      >
        {on ? '등록' : '미등록'}
      </span>
    );
  ...
  if (col.key === 'awsAccountIds') {
    ...
    return (
      <div className="flex flex-nowrap items-center gap-1 overflow-hidden">
        <span className="shrink-0 rounded bg-blue-50 px-1.5 py-0.5 font-mono text-[10px] text-blue-600 whitespace-nowrap">
          {first}
        </span>
    ...
    return (
      <div className="flex flex-nowrap items-center gap-1 overflow-hidden">
        {shown.map((n) => (
          <span
            key={n}
            className="shrink-0 rounded bg-zinc-100 px-1.5 py-0.5 text-[11px] font-medium text-zinc-700 whitespace-nowrap"
          >
            {n}
          </span>
        ))}
  ...
  if (col.type === 'dynamic-select')
    return resolveDisplayName(displayValue, col, opts.dynamicOptions) || '-';
  if (col.type === 'tags') {
    const tags = displayValue
      .split(',')
      .map((s) => s.trim())
      .filter(Boolean);
    ...
    return (
      <div className="flex flex-nowrap items-center gap-1 overflow-hidden">
        {shown.map((t) => (
          <span
            key={t}
            className={`shrink-0 rounded px-1.5 py-0.5 text-[11px] font-medium whitespace-nowrap ${MSP_TAG_COLORS[t] ?? 'bg-zinc-100 text-zinc-600'}`}
          >
            {t}
          </span>
        ))}
...
export function renderEditingCell(
  ...
  if (col.type === 'tags') {
    return (
      <InlineTagSelect
        value={opts.tempValue}
        onChange={opts.setTempValue}
        onDone={() => opts.saveCellEdit(row)}
      />
    );
  }
  const cellClass =
    'h-8 w-full rounded border border-blue-400 bg-blue-50 px-2 text-[13px] text-zinc-900 outline-none';
  ...
  if (col.type === 'number') {
    return (
      <MoneyInput
        ...
        inputMode="numeric"
        value={opts.tempValue}
        onValueChange={opts.setTempValue}
        onBlur={() => opts.saveCellEdit(row)}
        onKeyDown={onKeyDown}
        className={cellClass}
      />
